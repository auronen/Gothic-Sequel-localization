	...
	if (!(typ.lp < AtrPlus))
	{
		if (Typ_Atr + AtrPlus <= 100)
		{
			...
			if (ATR == ATR_STRENGTH)
			{
				...
				PrintString = ConcatStrings(NAME_RaiseStrength, AtrPlusSTRING);
				_ = PrintScreen(PrintSTRING, -1, -1, FONT_OLD_BIG, 1);
			}
			else if (ATR == ATR_DEXTERITY)
			{
				typ.attribute[ATR_DEXTERITY] = typ.attribute[ATR_DEXTERITY] + AtrPlus;
				PrintString = ConcatStrings(NAME_RaiseDexterity, AtrPlusSTRING);
				_ = PrintScreen(PrintSTRING, -1, -1, FONT_OLD_BIG, 1);
			}
			else if (ATR == ATR_MANA_MAX)
			{
				typ.attribute[ATR_MANA_MAX] = typ.attribute[ATR_MANA_MAX] + AtrPlus;
				PrintString = ConcatStrings(NAME_RaiseMana, AtrPlusSTRING);
				_ = PrintScreen(PrintString, -1, -1, FONT_OLD_BIG, 1);
			};
		}
		else
		{
			_ = PrintScreen(PRINT_MAX_100, -1, -1, FONT_OLD_BIG, 1);
			B_Say(self, other, "$NOLEARNOVERMAX");
		};
	}
	else
	{
		_ = PrintScreen(PRINT_NotEnoughSkillPoints, -1, -1, FONT_OLD_BIG, 1);
