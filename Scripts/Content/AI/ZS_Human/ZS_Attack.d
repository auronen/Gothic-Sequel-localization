...
func int ZS_Attack_Loop()
{
	...
	if (!C_NpcIsInvincible(other)) 																//MH: wird z.B. im Dialog auf True gesetzt, d.h. verfolgter SC kann sich unterhalten, ohne von aNSC angegriffen zu werden
	{
		AI_Attack(self);
	}
	else
...
func void ZS_Attack_End()
{
	...
	if (self.aivar[AIV_PLAYERISINROOM] == TRUE)
	{
...
func void B_CombatRemoveWeapon()
{
	...
	if (Npc_WasInState(self, ZS_AssessFighter) || Npc_WasInState(self, ZS_AssessFighterWait)) 	// Angriffs-Grund das Nichtbefolgen des "Waffe weg"?...
	{
		...
		if (C_NpcHasFullPower(self)) 															// ...und wurde der NSC noch nicht getroffen...
		{
