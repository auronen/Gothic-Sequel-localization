...
func void ZS_ObserveSuspect()
{
	...
	B_PointAndShout(self, other, "$WHYAREYOUSNEAKING");
...
func int ZS_ObserveSuspect_Loop()
{
	...
	{
		PrintDebugNpc(PD_ZS_LOOP, "... loop end, Cause: Too Far");
		return LOOP_END;
	};

	// [KARK] 02.08.2001: neu seit 1.4b, Npc_GetWalkMode()
	//if (!C_BodyStateContains(other, BS_SNEAK)) old
	//	Jp: Entfernunng sollte egal sein a) wer schleicht macht sich verdächtig b) optik im Spiel, es ist nicht mehr so deutlich, daß vom Wp an ein Mobsi gegangen wird etc.
	//if (Npc_GetWalkMode(other) != NPC_SNEAK)
	//{
		//PrintDebugNpc(PD_ZS_LOOP, "... loop end, Cause No more Sneaking");
		//		return LOOP_END;
	//};
