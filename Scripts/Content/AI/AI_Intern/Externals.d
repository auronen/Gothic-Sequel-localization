...
func void PrintScreenColored(var string msg, var int posx, var int posy, var string font, var int timeSec, var int color) { };
// Wie PrintScreen nur mit der Möglichkeit die Farbe der Textausgabe zu beeinflussen
// Farben wie folgt:
// TEXT_COLOR_WHITE
// TEXT_COLOR_GRAY
// TEXT_COLOR_RED
...
// TEXT_COLOR_BLUE
// TEXT_COLOR_YELLOW
func void AI_PrintScreen(var C_Npc n0, var string msg, var int posx, var int posy, var string font, var int timeSec, var int color) { };
// wie PrintScreenColored, nur über die AI-Queue, und der obligatorische self-Parameter
...
// Füge NSC in Welt ein. Wobei Spawnpoint  ein FP oder WP sein darf.
...
// Liefert den State zum nächsten Mob mit 'schemeName' zurück bzw. '-1' wenn kein solches Mob gefunden wurde !!!! Dies gilt tatsächlich für das nächste (räumlich) Mobsi
// und ist unabhängig davon, ob die Npc instanz an diesem Mobsi angemeldet ist.
func int Wld_GetInteractMobState(var C_Npc self, var string schemeName);
// Liefert den State des aktuellen Interact-Mob mit dem schemeName des self-NPC zurück, bei schemeName = "NULL" wird der schemeName ignoriert
func int Npc_GetMobGuild(var C_Npc other);
// Liefert die Gildenkonstante des Mobsis zurück, das von 'other' gerade benutzt wird
...
// -------------------------------------------------------------------------------------------
func void Npc_GetWalkMode(var C_Npc) { };
// liefert den aktuellen Walkmode (s.o.) des NSCs zurück, als Ersatz für BodyState bei ZS_OBSERVESUSPECT gedacht
...
// Sucht sich einen Freepoint im Umkreis von 7m vom NSC, bewegt sich dorthin und richtet sich entsprechend aus.
...
// Sucht einen Freepoint im Umkreis von NPC_MAX_FP_DIST (Balancing.d) Metern vom NSC und liefert TRUE falls vorhanden und frei ('self' zählt als Blockierer nicht!) und sichtbar
...
func int Wld_IsFPAvailInRange(var C_Npc self, var string fpName, var float range, ) { };
// Sucht einen Freepoint im Umkreis von 'range' Metern  vom NSC und liefert TRUE falls vorhanden und frei ('self' zählt als Blockierer nicht!) und sichtbar
func int Wld_IsNextFPAvailInRange(var C_Npc self, var string fpName, var float range, ) { };
//  wie Wld_IsFPAvailInRange(), aber es wird immer der nahegelegenste genommen und 'self' zählt als Blockierer!
...
func void Wld_InsertItems(var int itemInstance, var int anzahl, var string spawnPoint) { };
// Füge "anzahl" Items in Welt ein entweder an einem WP oder einem FP, ACHTUNG: funktioniert nur bei stapelbaren Items wie Münzen usw.
...
// !!! Die Slots beginnen bei 0 nicht bei eins zur Verwendung siehe B_Plunder
...
func void Npc_ClearInventory(var C_Npc self) { };
// Entfernt alle Items aus dem Inventory bis auf Items mit dem Flag ITEM_MISSION
...
func void AI_Snd_Play(var C_Npc self, var string s0) { };
// AI-Analogon zu SndPlay()
func void AI_Snd_Play3D(var C_Npc self, var C_Npc emitter, VAR_STRING s0) { };
// AI-Analogon zu SndPlay3D(), 'emitter' gibt hier die Quelle des Sounds an
