...
func void B_SetArenaPerception(var C_Npc slf)
{
	Npc_SetPercTime(slf, 1);

	Npc_PercEnable(slf, PERC_ASSESSMURDER, B_Arena_AssessMurder);
	Npc_PercEnable(slf, PERC_ASSESSFIGHTSOUND, B_Arena_FightSound);
	Npc_PercEnable(slf, PERC_ASSESSDEFEAT, B_Arena_AssessDefeat);
	Npc_PercEnable(slf, PERC_ASSESSDAMAGE, B_Arena_ReactToDamage);
	Npc_PercEnable(slf, PERC_DRAWWEAPON, B_Arena_AssessFighter);
	Npc_PercEnable(slf, PERC_ASSESSCASTER, B_Arena_AssessCaster);
